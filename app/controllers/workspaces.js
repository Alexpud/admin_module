  ...
  db.Workspace.findOne({
    ...
    .then((workspace) => {
      if( workspace != null ){
        ...
        var promise = new Promise(function (resolve, reject) {
          request({
            url: 'http://localhost:'+containerPort+'/api/workspace/'+workspace.workspaceID+'/runtime',
             method: 'POST',
             headers: {'Content-Type': 'application/json'},
...
router.delete('/containers/:containerName/workspaces/:workspaceName/stop', (req, res, next) =>{
  db.Workspace.findOne({
    ...
    .then((workspace) =>{
      var promise = new Promise((resolve, reject) =>{
        let containerPort = workspace.Container.port;
        exec('curl -H "Content-Type: application/json" -X "DELETE" http://localhost:'+containerPort+'/api/workspace/'+workspace.workspaceID+'/runtime?Password=password',
          function(err,stdout,stderr)
          {
            resolve({ response: stdout  });
          });
